...
Parser.buildHTMLFromJSON = function(data, board, standalone, fromQuote) {
    ...
    if (isOP) {
        ...
        if (fromQuote && data.replies) {
            ...
            if (data.images) {
                postCountStr += ' and ' + data.images + ' image' + (data.images > 1 ? 's' : '');
            }
    ...
    container.innerHTML = (isOP ? '' : '<div class="sideArrows" id="sa' + data.no + '">&gt;&gt;</div>') + '<div id="p' + data.no + '" class="post ' + postType + highlight + '">' + '<div class="postInfoM mobile" id="pim' + data.no + '">' + '<span class="nameBlock' + capcodeClass + '">' + mName + tripcode + capcodeStart + capcode + userId + flag + '<br>' + subject + '</span><span class="dateTime postNum" data-utc="' + data.time + '">' + data.now + ' <a href="' + data.no + '#p' + data.no + '" title="Link to this post">No.</a>' + '<a href="javascript:quote(\'' + data.no + '\');" title="Reply to this post">' + data.no + '</a></span>' + '</div>' + (isOP ? fileHtml : '') + '<div class="postInfo desktop" id="pi' + data.no + '"' + (board != Main.board ? (' data-board="' + board + '"') : '') + '>' + '<input type="checkbox" name="' + data.no + '" value="delete"> ' + subject + '<span class="nameBlock' + capcodeClass + '">' + emailStart + '<span class="name">' + name + '</span>' + tripcode + capcodeStart + emailEnd + capcode + userId + flag + ' </span> ' + '<span class="dateTime" data-utc="' + data.time + '">' + data.now + '</span> ' + '<span class="postNum desktop">' + '<a href="' + noLink + '" title="Link to this post">No.</a><a href="' + quoteLink + '" title="Reply to this post">' + data.no + '</a> ' + threadIcons + replySpan + '</span>' + '</div>' + (isOP ? '' : fileHtml) + '<blockquote class="postMessage" id="m' + data.no + '">' + (data.com || '') + capcode_replies + summary + '</blockquote> ' + '</div>' + mobileLink;
    if (!Main.tid || board != Main.board) {
...
QR.show = function(tid) {
    ...
    cnt.innerHTML = '<div id="qrHeader" class="drag postblock">Reply to Thread No.<span id="qrTid">' + tid + '</span><img alt="X" src="' + Main.icons.cross + '" id="qrClose" ' + 'class="extButton" title="Close Window"></div>';
...
var Linkify = {
    ...
    exec: function(el) {
        ...
        el.innerHTML = el.innerHTML.replace(/<wbr>/g, '\u200b').replace(this.linkRe, this.funk).replace(/\u200b/g, '<wbr>');
    },
...
Report.open = function(pid, board) {
    window.open('https://sys.4chan.org/' + (board || Main.board) + '/imgboard.php?mode=report&no=' + pid, Date.now(), "toolbar=0,scrollbars=0,location=0,status=1,menubar=0,resizable=1,width=600,height=270");
};
/**
 * Custom Menu
 */
...
var Draggable = {
    ...
    startDrag: function(e) {
        ...
        if (Config.dropDownNav && !Config.autoHideNav) {
            self.offsetTop = $.id(Config.classicNav ? 'boardNavDesktop' : 'boardNavMobile').offsetHeight;
        } else {
            self.offsetTop = 0;
        }
...
Main.onThreadMouseOut = function(e) {
    ...
    } else if (Config.imageHover && (t.hasAttribute('data-md5') || (t.href && !$.hasClass(t.parentNode, 'fileText') && /i\.4cdn\.org\/[a-z0-9]+\/[0-9]+\.(gif|jpg|png|webm)$/.test(t.href)))) {
        ImageHover.hide();
    } else if ($.hasClass(t, 'dateTime')) {
